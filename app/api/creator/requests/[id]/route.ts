...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}) {
 ...
 const limited=await enforceRateLimit(request,'requestAction',viewer.id);if(limited)return limited;
 ...
 await notifyPaymentLifecycle(p,action==='accept'?'accepted':'declined');
